...
\begin{document}
   ...
   %FINEFRASE
	...
	\chapter{Rete neurale artificiale}
		...
		La breve illustrazione del sistema neurale umano nel paragrafo precedente suggerisce uno schema per delineare l'organizzazione delle reti neurali artificiali: queste ultime sono formate da un elevato numero di unità computazionali, che possono essere equiparate ai neuroni umani, e che \textbf{(toglierei il che)} sono capaci di eseguire una somma pesata. Tali unità sono collegate tra loro attraverso delle connessioni, così come le sinapsi collegano i neuroni nella rete umana. \\
		...
		\textbf{Qua farei una cosa del genere (o comunque qualcosa di simile se ti piace) i valori di 2 e 3 cm nel codice sono empirici e sperimentali xD}
		\begin{center}
			\begin{tabular}{ccr}
			\textbf{net:} \hspace{2cm} & $\sum\limits_{i=1}^n w_{i}x_{i}$ & \hspace{3cm} \\
			\end{tabular}
		\end{center}

		%qua sopra ho fatto una tabella \begin{tabular} i "ccr" stanno a significare che fai 3 colonne (3 sono le lettere usate) e che la prima e la seconda colonna devono avere il contenuto centrato mentre la terza colonna sulla destra. Separi il contenuto da una colonna all'altra con & mentre le due \\ dicono che è finita la riga della tabella.
		...
		Interpretando la soglia come il peso associato ad un ulteriore canale di ingresso $x_{0}$, di valore sempre costante e pari a $-1$, potremmo anche scrivere, $w_{0}=\theta$: 
		...
		MANNAGGIA TUTTI GLI ELENCHI CHE NON ENTRANO NELLE PAGINE! COME POSSO FARE?
		
		SEI UN CICCINO!
		
\textbf{NON TI PREOCCUPARE SE LE COSE VENGONO SPEZZATE. TE GUARDA IL CODICE E CAPISCI COME STANNO MESSE LE COSE. QUANDO HAI FINITO DI SCRIVERE SI RIVEDE TUTTO QUANTO PER SISTEMARE, TRANQUILLA. BASTA UN CODICETTO QUA E LA E TUTTO SI SISTEMA. SECONDO ME HAI CONFUSO LE DUE IMMAGINI PALLA2 E PALLA3, CONTROLLA.}
