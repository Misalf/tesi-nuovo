...
%INIZIO DEL SECONDO CAPITOLO%
	...
	\section {Processi di apprendimento}
		 ...
		 \begin{equation}  
			 $$ \begin{center} $\delta_{k}=y_{dk}-y_{k} $ \end{center} $$
		 \end{equation} 
		 ...
		 \begin{equation}
		 	$$\begin{center} $e(t)= y_{dk}(t)-y_{k}(t)$ \end{center}$$
		 \end{equation}
		...
		\begin{equation}
		 $$\begin{center}
		 	 $\frac{dE(\overline{w})}{d\overline{w}}=\nabla_ {w}E(\overline{w})=( \frac{dE(\overline{w})}{\partial(w_{1})}, \frac{dE(\overline{w})}{\partial(w_{2})}, ... ,\frac{dE(\overline{w})}{\partial(w_{n})} )^{T}$
		 \end{center}$$
		\end{equation}
	 	...
	 	\begin{equation}
	 		$$\begin{center}
	 		$E_{k}=\frac{1}{2}(y_{dk}-y_{k})^{2}$
	 		\end{center}$$
	 	\end{equation}
	 	
		 Ma allora se consideriamo l'$i$-esimo peso del $k$-esimo pattern si avrà che:
		 ...
		 \begin{equation}
		 	...
		 	\dfrac{\partial}{\partial w_{i}}[\frac{1}{2}(y_{dk}-y_{k})^{2}] =
		 	(y_{dk}-y_{k}) \frac{\partial y_{dk}-y_{k}}{\partial w_{i}} =
		 	(y_{dk}-y_{k})\frac{\partial y_{dk}}{\partial w_{i}}=
		 	(y_{dk}-y_{k})\frac{\partial g(net)}{\partial w_{i}}=
		 	(y_{dk}-y_{k})\frac{\partial g(net)}{\partial net_{k}}\frac{\partial net}{\partial w_{i}}=
		 ...
		 \end{center}$$
		 	...
		 	\begin{equation}
		 		$$\begin{center} $\triangle{w_{i}}=\eta\delta_{k} g'(net_{k}) x_{i}$ 
		 		\end{center}$$	
		 	\end{equation}
		...
		\subsection{Apprendimento competitivo}
		 ...
		 Tale apprendimento si basa su una vera a propria competizione tra i neuroni di uscita di una rete neurale: questi competono tra loro per attivarsi in seguito ad uno stimolo. In un certo momento $t$ può attivarsi un solo neurone che viene denominato \textbf{winners-takes-all}.\\  
		 ...
		 Nella forma più semplice la rete neurale ha un solo strato di neuroni di uscita, completamente connessi ai nodi di input e possono essere presenti connessioni tra i neuroni che portano ad inibizioni laterali.\\
		 ...
		 Il neurone $k$ che si attiva è quello con input netto $\nu_{k}$ più alto per un dato input $x$, con $\nu_{k}$ combinazione lineare di tutti gli input. Il suo segnale di output $y_{dk}$ sarà $1$, mentre quello degli altri neuroni rimasti inattivi è $0$.
