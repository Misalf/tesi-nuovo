...
%INIZIO DEL SECONDO CAPITOLO%
	...
	\section {Processi di apprendimento}
		 ...
		 \subsection{Delta rule o regola di Widrow-Hoff}
		 
		  \subsection{Apprendimento di Hebbian}
		 
		 \subsection{Apprendimento con correzione di errore o metodo discesa del gradiente}
		 
		 Se indichiamo con $y_{k}(t)$ una risposta generata da un segnale di stimolo $x(t)$ in un tempo $t$ ed indichiamo con $y_{dk}(t)$ la risposta desiderata, avremo un \emph{segnale di errore} che può essere così espresso:\\
		 ...
		 Per la scelta di tali pesi si sfruttano le informazioni fornite dal gradiente locale della funzione errore costruita, che suggeriscono una proporzionalità.\\
		 
		 %figura
		 
		 Se $\nabla_ {w}E(\overline{w})$
	...
	\section{Un esempio di apprendimento supervisionato: il percettrone elementare}
		...
		L'apprendimento utilizzato nel perceptron è di tipo supervisionato e la learning rule attraverso i quali si modificano i singoli pesi in modo appropriato è la delta rule.\\
		I passi da eseguire sono:
		
		\begin{itemize}
			\item {1) Si inizializzano i pesi con valori casuali};
			\item {2) Si presenta un ingresso $x_{k}$ ed il valore di uscita desiderato $y_{k}$} ;
			\item {3) Si calcola la risposta $y_{dk}$ e si aggiornano i pesi attraverso la delta rule;}
