...
\begin{document}
   ...
   %FINEFRASE
	...
	Per costruire una rete neurale efficiente, un passo fondamentale è individuare un \emph{insieme di apprendimento} ed un \emph{algoritmo di apprendimento}.\\
	Per insieme di appredimento si intende una collezione di esempi dai quali essa può attingere per raggiungere lo scopo per il quale è stata progettata; per algoritmo, invece, un procedimento che permetta di prelevare le informazioni dall'insieme di apprendimento fissandone dei parametri, e che modifichi questi ultimi attraverso operazioni iterative interfaccianti con l'ambiente.
	Ciò ci suggerisce assieme al tratto caratteristico del sistema nervoso a cui le reti si ispirano, ovvero la capacità di acquisire esperienza da esempi del mondo reale, che la rete viene \emph{addestrata} mediante gli algoritmi di apprendimento e gli esempi presenti nell'insieme di apprendimento.
