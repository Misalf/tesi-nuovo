...
\begin{document}
   ...
   %FINEFRASE
	...
	\chapter{Rete neurale artificiale}
		...
		La breve illustrazione del sistema neurale umano nel paragrafo precedente suggerisce uno schema per delineare l'organizzazione delle reti neurali artificiali: queste ultime sono formate da un elevato numero di unità computazionali, che possono essere equiparate ai neuroni umani, e che sono capaci di eseguire una somma pesata. Tali unità sono collegate tra loro attraverso delle connessioni pesate, così come le sinapsi collegano i neuroni nella rete umana. \\
		Il neurone artificiale è l'unità di calcolo fondamentale della rete neurale. Gli elementi base che lo rappresentano sono:
		\begin{itemize}
			\item Un insieme di connessioni ciascuna delle quali è caratterizzata da un \emph{peso} che può essere sia positivo che negativo e che rappresenta l'\emph{efficacia sinaptica}, ovvero la forza della connessione;
			\item Una funzione di attivazione;
			\item Un valore di soglia;
			\item Un sommatore.
		\end{itemize}
		
		L'insieme di connessione è formato da $n$ canali di ingresso $x_{1}, x_{2}, ... ,x_{n}$ all'interno dei quali  $y$ \theta
		
		
		 2. un sommatore che somma i segnali in input pesati dalle rispettive sinapsi, producendo in output una combinazione lineare degli input; 3. una funzione di attivazione per limitare l'ampiezza dell'output di un neurone. Tipicamente per comodità l'ampiezza degli output appartengono all'intervallo [0,1] oppure [-1,1]. Il modello neuronale include anche un valore soglia che ha l'effetto, a seconda della sua positività o negatività, di aumentare o diminuire l'input netto alla funzione di attivazione.
		
		2)  Un neurone artificiale è rappresentato da:
	
		na soglia
		na funzione di attivazione
		L'effetto di un segnale x sul neurone postsinaptico è semplicemente uguale al prodotto w · x, dove w è il peso attribuito alla sinapsi corrispondente. Il potenziale di attivazione è dato dalla somma algebrica dei prodotti di tutti i segnali di ingresso xj e i valori dei pesi corrispondenti wij 
